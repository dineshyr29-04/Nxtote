...
function Home() {
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case "High": return "text-rose-400 bg-rose-500/10 border-rose-500/20 shadow-sm shadow-rose-500/5";
        ...
        <div className="flex gap-4">
          <div className="bg-slate-900/40 border border-slate-800/80 px-6 py-4 rounded-2xl flex items-center gap-4 min-w-[200px] backdrop-blur-md">
            ...
            <div>
              ...
              <p className="text-lg font-extrabold text-slate-100">{completedTasks} / {totalTasks}</p>
            </div>
          </div>
          ...
          <div className="bg-slate-900/30 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-xl hover:border-slate-850 transition duration-300">
            ...
            <form onSubmit={handleAddTask} className="flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                {/* Priority select */}
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Priority</label>
                  <select
          ...
          <div className="bg-slate-900/10 border border-slate-900/80 rounded-3xl p-6 min-h-[420px] backdrop-blur-sm flex flex-col">
            ...
            {filteredTasks.length === 0 ? (
              <div className="flex flex-col items-center justify-center my-auto py-16 text-center">
                ...
                <p className="text-slate-400 font-medium">Clear of outstanding tasks!</p>
                <p className="text-xs text-slate-500 mt-1">Try relaxing, or add a new action item to the board.</p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-semibold truncate ${task.completed ? "line-through text-slate-500" : ""}`}>
                          {task.text}
                        </p>
                        <div className="flex items-center gap-2 mt-2 flex-wrap">
                          ...
                          <span className={`text-[9px] px-2 py-0.5 rounded-md border font-extrabold uppercase ${getPriorityColor(task.priority)}`}>
                            {task.priority}
                          </span>
